...
<%--				&nbsp;&nbsp;--%>
                <A HREF="<c:out value='${topicBean.replyTopicUrl}'/>&reply=true"><IMG SRC="<%= application.getRealPath("/") %>/riverock/forum/img/reply.gif" WIDTH="98" HEIGHT="22" BORDER="0" ALT="reply"></A>
		</td>
	</tr>
</table>
...
<!--loop begin-->
	...
	<tr>
		...
		<td class="<c:out value='${backgroundStyle}'/>" valign="top" cellpadding="5">
			...
			&nbsp;&nbsp;<img name="avatar" src="<%= application.getRealPath("/") %>/riverock/forum/img/avatars/<c:out value='${message.u_avatar_id}'/>.gif" width="36" height="36" border="0"><BR>
            </c:if>
		...
		<td class="<c:out value='${backgroundStyle}'/>">
				...
				<td>
                    ...
                    <c:if test="${!empty message.m_iconid && message.m_iconid > 0}">
                        <img src="<%= application.getRealPath("/") %>/riverock/forum/img/icons/<c:out value='${message.m_iconid}'/>.gif" width="15" height="15" border="0">
                    </c:if>
                ...
                <A HREF="<c:out value='${genericBean.forumHomeUrl}'/>&action=edit-message&t_id=<c:out value='${topicBean.t_id}'/>&f_id=<c:out value='${topicBean.f_id}'/>&m_id=<c:out value='${message.m_id}'/>">
                    <IMG SRC="<%= application.getRealPath("/") %>/riverock/forum/img/m_edit.gif" WIDTH="55" HEIGHT="17" BORDER="0" ALT="">
                </A>
                </c:when>
                <c:otherwise>
                    <IMG SRC="<%= application.getRealPath("/") %>/riverock/forum/img/m_edit.gif" WIDTH="55" HEIGHT="17" BORDER="0" ALT="">
...
<%--						<A HREF="user.do?u_id=<%=message.getM_u_id()%>">--%>
                            <IMG SRC="<%= application.getRealPath("/") %>/riverock/forum/img/m_profile.gif" WIDTH="55" HEIGHT="17" BORDER="0" ALT="">
<%--                            </A>--%>
							<IMG SRC="<%= application.getRealPath("/") %>/riverock/forum/img/m_pm.gif" WIDTH="55" HEIGHT="17" BORDER="0" ALT="">
							<IMG SRC="<%= application.getRealPath("/") %>/riverock/forum/img/m_email.gif" WIDTH="55" HEIGHT="17" BORDER="0" ALT="">
							<IMG SRC="<%= application.getRealPath("/") %>/riverock/forum/img/m_www.gif" WIDTH="55" HEIGHT="17" BORDER="0" ALT="">
							<IMG SRC="<%= application.getRealPath("/") %>/riverock/forum/img/m_search.gif" WIDTH="55" HEIGHT="17" BORDER="0" ALT="">
						</td>
					</tr>
				</table>
		</td>
