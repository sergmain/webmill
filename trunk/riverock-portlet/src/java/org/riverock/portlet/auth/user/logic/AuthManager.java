...
import org.riverock.portlet.auth.user.bean.CompanyItemBean;
...
public class AuthManager implements Serializable {
    ...
    private List<CompanyBean> initCompanyBeans() {
        ...
        String userLogin = moduleUserBean.getUserLogin();
                ...
                try {
                    ...
                    while( rs1.next() ) {
                        ...
                        if( userBean.getCompanyId() != null ) {
                            WmListCompanyItemType companyItem = GetWmListCompanyItem.getInstance( db_,
                                userBean.getCompanyId() ).item;
                            userBean.setCompanyName( companyItem.getShortName() );
                        }
                        if( userBean.getGroupCompanyId() != null ) {
                            WmListGroupCompanyItemType groupCompanyItem = GetWmListGroupCompanyItem.getInstance( db_,
                                userBean.getGroupCompanyId() ).item;
                            userBean.setGroupCompanyName( groupCompanyItem.getShortNameService() );
                        }
                        if( userBean.getHoldingId() != null ) {
                            WmListHoldingItemType holdingItem = GetWmListHoldingItem.getInstance( db_,
                                userBean.getHoldingId() ).item;
    ...
    private List<RoleBean> getRoles( DatabaseAdapter db_, Long authUserId ) throws Exception {
        ...
        ResultSet rs = null;
        try {
            ps = db_.prepareStatement( "select c.ID_ACCESS_GROUP, c.NAME_ACCESS_GROUP " +
    ...
    private void processDeleteUserRoles( DatabaseAdapter db_, Long authUserId ) throws Exception {
        PreparedStatement ps = null;
        try {
            ps = db_.prepareStatement( "delete from WM_AUTH_RELATE_ACCGROUP " +
                "where  ID_AUTH_USER=? " );
    ...
    private void processDeletedRoles( DatabaseAdapter db_, UserBean userBean ) throws Exception {
        PreparedStatement ps = null;
        try {
            log.info( "Start delete roles for authUserId: " + userBean.getAuthUserId() +
            ...
            if( isRigthRelate ) {
                ...
                switch( db.getFamaly() ) {
                    ...
                    default:
                        ...
                        RsetTools.setLong( ps, 1, authUserId );
                        ps.setString( 2, moduleUserBean.getUserLogin() );
            ...
            if( isRigthRelate ) {
                ...
                switch( db.getFamaly() ) {
                    ...
                    default:
                        ...
                        RsetTools.setLong( ps, 9, id_auth_user );
                        ps.setString( 10, moduleUserBean.getUserLogin() );
    ...
    public void processAddUser( UserBean userBean ) {
        ...
        try {
            ...
            ps.setInt( 8,
                moduleUserBean.isCompany()
                ? userBean.isCompany() ? 1 : 0
                : 0 );
            ps.setInt( 9,
                moduleUserBean.isGroupCompany()
                ? userBean.isGroupCompany() ? 1 : 0
                : 0 );
            ps.setInt( 10,
                moduleUserBean.isHolding()
                ? userBean.isHolding() ? 1 : 0
                : 0 );
            int i1 = ps.executeUpdate();
...
// List's methods
    ...
    public List<RoleBean> getRoleList() {
        ...
        try {
            db = DatabaseAdapter.getInstance();
            ps = db.prepareStatement( "select  ID_ACCESS_GROUP, NAME_ACCESS_GROUP " +
            ...
            switch( db.getFamaly() ) {
                ...
                default:
                    ps = db.prepareStatement( "select b.ID_FIRM, B.FULL_NAME NAME_FIRM " +
                        ...
                        "order  by b.ID_FIRM ASC " );
                    ps.setString( 1, moduleUserBean.getUserLogin() );
                    break;
            }
            rs = ps.executeQuery();
            ...
            switch( db.getFamaly() ) {
                case DatabaseManager.MYSQL_FAMALY:
                    String sql_ =
                        ...
                        // mysql create timestamp field with presseted value
                        // read 1.8.6.2 Constraint NOT NULL and DEFAULT Values 'mysql manual'
