...
<%@ taglib uri="http://myfaces.apache.org/tomahawk" prefix="t"%>
    ...
    <f:loadBundle basename="org.riverock.portlet.manager.resource.Holding" var="msg"/>	
        ...
        <h:commandButton id="add-user-action" action="#{holdingAction.addUserAction}"
...
<%/*    <h:commandLink value="Expand All" action="#{userTree.expandAll}"/> */%>
    ...
    <t:tree2 id="serverTree" value="#{userTree.userTree}" var="node" varNodeToggler="t" clientSideToggle="false" showRootNode="false" binding="#{userTree.tree}">
        ...
        <f:facet name="user">
			...
			actionListener="#{holdingAction.selectUserAction}">

                    <t:graphicImage value="/images/user.png" border="0"/>
                    <h:outputText value="#{node.description}"/>

                    <t:updateActionListener property="#{holdingSessionBean.currentAuthUserId}" value="#{node.identifier}" />
                </h:commandLink>
            </h:panelGroup>
        </f:facet>
    </t:tree2>
