...
//                (CtxInstance)request_.getSession().getAttribute( org.riverock.webmill.main.Constants.PORTLET_REQUEST_SESSION );
            ...
            if ( log.isDebugEnabled() )
            {
                for ( Enumeration e = renderRequest.getParameterNames(); e.hasMoreElements(); )
                {
                    ...
                    log.debug( "PortletRequest attr - "+s+", value - "+PortletTools.getString( renderRequest, s ) );
                }
            }
            Long id_lang = PortletTools.getLong(renderRequest, Constants.NAME_ID_LANGUAGE);
