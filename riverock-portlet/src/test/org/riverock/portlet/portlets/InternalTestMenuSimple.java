...

{
    ...
    private static Logger log = Logger.getLogger("org.riverock.portlet.portlets.InternalTestMenuSimple");
...
//        Assert.assertEquals(true, menu.getId()!=-1);
        ...
        Assert.assertEquals(true, menu.param.getJspPage()!=null);



        for (int j = 0; j < defMenu.getMenuItemCount(); j++)

        {

            MenuItemInterface ci = defMenu.getMenuItem(j);



            MenuModuleType tempMenu = menu.getMenuModule(

                    ci, menu.param.getResponse(), menu.param.getJspPage(), 1, menu.getId(), desc, portletType
